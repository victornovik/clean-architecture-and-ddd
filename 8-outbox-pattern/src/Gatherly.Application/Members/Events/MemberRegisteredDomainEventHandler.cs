...
internal sealed class MemberRegisteredDomainEventHandler(IMemberRepository memberRepository, IEmailService emailService)
    : IDomainEventHandler<MemberRegisteredDomainEvent>
{
    ...
    {
        Member? member = await memberRepository.GetByIdAsync(notification.MemberId, cancellationToken);
        if (member is null)
        {
            return;
        }
